...
import { FiDownload, FiFileText, FiInfo, FiCalendar, FiTag, FiBarChart2, FiUser, FiFilter } from 'react-icons/fi';
...
const THEMES = {
  dark: {
    ...
    accent: '#2563eb', accentEnd: '#1e40af', accentShadow: 'rgba(14,165,233,0.4)',
    primaryBg: 'linear-gradient(135deg, #2563eb, #1e40af)',
    inputBg: '#111111', inputBorder: '#333333', inputColor: '#E1E1E1',
    divider: 'rgba(255,255,255,0.06)',
    listItemBg: '#111111', listItemBorder: '#222222',
    adminBoxBg: 'rgba(34,197,94,0.08)', adminBoxBorder: 'rgba(34,197,94,0.25)', adminTitle: '#22c55e', adminText: '#86efac',
  ...
  light: {
    ...
    listItemBg: '#f8fafc', listItemBorder: 'rgba(0,0,0,0.06)',
    adminBoxBg: '#dcfce7', adminBoxBorder: '#86efac', adminTitle: '#15803d', adminText: '#166534',
    infoIconBg: 'rgba(16,185,129,0.1)',
    secondaryBtn: 'rgba(0,0,0,0.04)', secondaryBorder: 'rgba(0,0,0,0.12)', secondaryColor: '#64748b',
  },
};

const F = { serif: "'DM Serif Display', Georgia, serif", sans: "'DM Sans', -apple-system, sans-serif" };
...
export default function ReportsPage() {
  ...
  // Stack cards on mobile/tablet, side-by-side on desktop
  const cardsGrid = isDesktop ? '1fr 1fr' : '1fr';
  ...
  const cardHd = { padding: isMobile ? '12px 16px' : '15px 20px', borderBottom: `1px solid ${T.divider}` };
  const inputSty = {
    width: '100%', padding: '11px 14px', borderRadius: 10,
    border: `1px solid ${T.inputBorder}`, background: T.inputBg, color: T.inputColor,
    fontSize: 14, fontFamily: F.sans, outline: 'none', transition: 'border 0.2s', boxSizing: 'border-box',
  };
  ...
  return (
    <div style={{ background: T.pageBg, minHeight: '100vh', padding: pagePadding, fontFamily: F.sans, transition: 'all 0.3s' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: isMobile ? 16 : 28 }}>
        <div>
          <h1 style={{ fontFamily: F.serif, fontSize: isMobile ? 22 : 30, color: T.title, letterSpacing: '-0.02em', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 10 }}>
            <FontAwesomeIcon icon={faFilePdf} style={{ fontSize: isMobile ? 20 : 26, color: T.accent }} />
            Reports
          </h1>
          <p style={{ color: T.subtitle, fontSize: isMobile ? 13 : 14, fontFamily: F.sans }}>
            Generate and export cattle stress detection reports
          </p>
        </div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: cardsGrid, gap: isMobile ? 14 : 24, alignItems: 'start' }}>

        {/* Export card */}
        <div style={card}>
          <div style={cardHd}>
            <h3 style={{ fontFamily: F.serif, fontSize: isMobile ? 16 : 18, color: T.title, display: 'flex', alignItems: 'center', gap: 8, margin: 0 }}>
              <FiDownload size={isMobile ? 15 : 17} style={{ color: T.accent, flexShrink: 0 }} />
              Export PDF Report
            </h3>
          </div>
          <div style={{ padding: isMobile ? '14px' : 20 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={labelSty}>FILTER BY DATE (OPTIONAL)</label>
              ...
              <small style={{ color: T.muted, fontSize: 12, marginTop: 4, display: 'block', fontFamily: F.sans }}>
            ...
            <div style={{ marginBottom: isMobile ? 20 : 24 }}>
              <label style={labelSty}>FILTER BY CATTLE ID (OPTIONAL)</label>
              <div style={{ position: 'relative' }}>
                <div style={{ position: 'absolute', left: 13, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }}>
                  <FiFilter size={14} style={{ color: T.muted }} />
                </div>
                <input
                  placeholder="e.g. 001" value={cattleId}
            ...
            <button
              onClick={downloadPDF}
              disabled={loading}
              style={{ ...primaryBtn, width: '100%', padding: isMobile ? '12px' : '13px', fontSize: isMobile ? 14 : 15, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9 }}
            >
              {loading
        ...
        <div style={card}>
          <div style={cardHd}>
            <h3 style={{ fontFamily: F.serif, fontSize: isMobile ? 16 : 18, color: T.title, display: 'flex', alignItems: 'center', gap: 8, margin: 0 }}>
              <FiInfo size={isMobile ? 15 : 17} style={{ color: T.accent, flexShrink: 0 }} />
              Report Information
            </h3>
          </div>
          <div style={{ padding: isMobile ? '14px' : 20 }}>
            <p style={{ color: T.muted, fontSize: 14, marginBottom: 16, fontFamily: F.sans }}>The PDF report includes:</p>
            <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
              {INFO_ITEMS.map((item, i) => (
                <li key={i} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '10px 14px', background: T.listItemBg, border: `1px solid ${T.listItemBorder}`, borderRadius: 10, fontSize: 13, fontFamily: F.sans, color: T.body }}>
                  <div style={{ width: 30, height: 30, borderRadius: 8, background: T.infoIconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: T.accent, flexShrink: 0 }}>
                    {item.icon}
                  </div>
                  {item.text}
                </li>
              ))}
              <li style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '10px 14px', background: T.listItemBg, border: `1px solid ${T.listItemBorder}`, borderRadius: 10, fontSize: 13, fontFamily: F.sans, color: T.body }}>
                <div style={{ width: 30, height: 30, borderRadius: 8, background: T.infoIconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: T.accent, flexShrink: 0 }}>
                  <FiUser size={15} />
                </div>
              ...
              <li style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '10px 14px', background: T.listItemBg, border: `1px solid ${T.listItemBorder}`, borderRadius: 10, fontSize: 13, fontFamily: F.sans, color: T.body }}>
                ...
                Filtered by date or cattle ID if specified
              </li>
            </ul>

            {user?.role === 'admin' && (
              <div style={{ marginTop: 18, background: T.adminBoxBg, border: `1px solid ${T.adminBoxBorder}`, borderRadius: 10, padding: '14px 16px', display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                ...
                <div>
                  ...
                  <p style={{ color: T.adminText, fontSize: 12, marginTop: 4, fontFamily: F.sans }}>You can view all farmers' records in this report.</p>
                </div>
              </div>
            )}
          </div>
