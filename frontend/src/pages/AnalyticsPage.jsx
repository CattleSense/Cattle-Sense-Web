...
import { FiSearch, FiSmile, FiMeh, FiAlertOctagon, FiBarChart2,
...
const THEMES = {
  dark: {
    ...
    chartGrid:'rgba(255,255,255,0.06)', chartTick:'#A0A0A0',
    barColor:'rgba(34,197,94,0.7)', barBorder:'#22c55e',
    lineColor:'#f97316', lineFill:'rgba(249,115,22,0.12)',
    statIconBg1:'rgba(34,197,94,0.15)', statIconBg2:'rgba(34,197,94,0.15)',
    statIconBg3:'rgba(234,179,8,0.15)', statIconBg4:'rgba(239,68,68,0.15)',
    rankBg1:'rgba(251,191,36,0.2)', rankBg2:'rgba(148,163,184,0.15)', rankBg3:'rgba(180,83,9,0.15)',
  },
  light: {
    ...
    chartGrid:'rgba(0,0,0,0.05)', chartTick:'#94a3b8',
    barColor:'rgba(34,197,94,0.65)', barBorder:'#22c55e',
    lineColor:'#f97316', lineFill:'rgba(249,115,22,0.08)',
    statIconBg1:'rgba(34,197,94,0.12)', statIconBg2:'rgba(34,197,94,0.12)',
    statIconBg3:'rgba(234,179,8,0.1)', statIconBg4:'rgba(239,68,68,0.1)',
    rankBg1:'rgba(251,191,36,0.15)', rankBg2:'rgba(148,163,184,0.1)', rankBg3:'rgba(180,83,9,0.1)',
  },
};
...
export default function AnalyticsPage() {
  ...
  const pieData = {
    labels: stressLabels,
    datasets: [{ data: stressCounts, backgroundColor: stressLabels.map(l => STRESS_CONFIG[l]?.color || '#ccc'), borderWidth: 2, borderColor: isDark ? '#1E1E1E' : '#fff' }],
  };
  const barData = {
    labels: months,
    datasets: [{ label: 'Detections', data: monthlyCnt, backgroundColor: T.barColor, borderColor: T.barBorder, borderWidth: 2, borderRadius: 7 }],
  };
  const lineData = {
    labels: months,
    datasets: [{ label: 'Avg Stress Level', data: monthlyAvg, borderColor: T.lineColor, backgroundColor: T.lineFill, borderWidth: 2, fill: true, tension: 0.4, pointBackgroundColor: T.lineColor, pointRadius: 4 }],
  };
  ...
  const chartOpts = (yLabel) => ({
    ...
    scales: {
      x: { grid: { color: T.chartGrid }, ticks: { color: T.chartTick, font: { family: F.sans, size: 11 } } },
      y: { beginAtZero: true, grid: { color: T.chartGrid }, ticks: { color: T.chartTick, font: { family: F.sans, size: 11 }, callback: yLabel } },
    },
  });
  ...
  <FiBarChart2 size={24} style={{ color: T.accent, flexShrink: 0 }} />
  Analytics
</h1>
  ...
  {[
    { icon: <FiSearch       size={20} />, label: 'Total Detections', value: data?.total || 0,                                                           iconBg: T.statIconBg1, iconColor: '#22c55e', color: '#22c55e' },
    { icon: <FiSmile        size={20} />, label: 'Calm',             value: data?.stressDist?.find(s => s._id === 'Calm')?.count || 0,                  iconBg: T.statIconBg2, iconColor: '#22c55e', color: '#22c55e' },
    { icon: <FiMeh          size={20} />, label: 'Moderate',         value: data?.stressDist?.find(s => s._id === 'Moderate')?.count || 0,              iconBg: T.statIconBg3, iconColor: '#eab308', color: '#eab308' },
    { icon: <FiAlertOctagon size={20} />, label: 'Extreme',          value: data?.stressDist?.find(s => s._id === 'Extreme')?.count || 0,               iconBg: T.statIconBg4, iconColor: '#ef4444', color: '#ef4444' },
  ].map((s, i) => (
    <div key={i} style={{ ...card, padding: '20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        <div style={{
          width: 46, height: 46, borderRadius: 12, background: s.iconBg,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        <div style={card}>
          <div style={{ padding: '16px 20px', borderBottom: `1px solid ${T.divider}` }}>
            <h3 style={{ fontFamily: F.serif, fontSize: 18, fontWeight: 700, color: T.title, display: 'flex', alignItems: 'center', gap: 8 }}>
  <FiPieChart size={17} style={{ color: T.accent, flexShrink: 0 }} />
  Stress Distribution
</h3>
          </div>
          <div style={{ padding: 20, display: 'flex', justifyContent: 'center' }}>
            {stressCounts.length > 0
              ? <div style={{ width: 280, height: 280 }}><Doughnut data={pieData} options={{ plugins: { legend: { position: 'right', labels: { color: T.body, font: { family: F.sans, size: 12 } } } }, cutout: '55%' }} /></div>
        ...
        <div style={card}>
          <div style={{ padding: '16px 20px', borderBottom: `1px solid ${T.divider}` }}>
           <h3 style={{ fontFamily: F.serif, fontSize: 18, fontWeight: 700, color: T.title, display: 'flex', alignItems: 'center', gap: 8 }}>
  <FiCalendar size={17} style={{ color: T.accent, flexShrink: 0 }} />
  Monthly Detections
</h3>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 20 }}>
        <div style={card}>
          <div style={{ padding: '16px 20px', borderBottom: `1px solid ${T.divider}` }}>
           <h3 style={{ fontFamily: F.serif, fontSize: 18, fontWeight: 700, color: T.title, display: 'flex', alignItems: 'center', gap: 8 }}>
  <FiTrendingUp size={17} style={{ color: T.accent, flexShrink: 0 }} />
  Average Stress Trend
        ...
        <div style={card}>
          <div style={{ padding: '16px 20px', borderBottom: `1px solid ${T.divider}` }}>
           <h3 style={{ fontFamily: F.serif, fontSize: 18, fontWeight: 700, color: T.title, display: 'flex', alignItems: 'center', gap: 8 }}>
  <FiAward size={17} style={{ color: '#ef4444', flexShrink: 0 }} />
            ...
            {data?.topCattle?.length > 0 ? data.topCattle.map((c, i) => {
              ...
              return (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 0', borderBottom: `1px solid ${T.divider}` }}>
                  <div style={{ width: 30, height: 30, borderRadius: '50%', background: rankBg, color: T.title, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, flexShrink: 0 }}>{i + 1}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 700, fontSize: 14, color: T.title }}>Cattle #{c._id}</div>
                    <div style={{ fontSize: 11, color: T.muted }}>{c.totalChecks} detections</div>
                  </div>
                  <span style={{ background: cfg?.bg, color: cfg?.text, padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 700 }}>{label}</span>
                </div>
              );
