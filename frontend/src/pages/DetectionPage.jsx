...
import {
  FiVideo, FiUpload, FiCamera, FiCircle, FiSquare, FiCheckCircle,
  FiClock, FiX, FiSearch, FiCpu, FiUploadCloud, FiCheckSquare,
  FiActivity, FiBarChart2, FiRefreshCw, FiClipboard,
} from 'react-icons/fi';
...
export default function DetectionPage() {
  ...
  const layoutGrid = isDesktop ? '1fr 1fr' : '1fr';
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: isMobile ? 16 : 28 }}>
        <div>
          <h1 style={{ fontFamily: F.serif, fontSize: isMobile ? 22 : 28, fontWeight: 900, color: T.title, letterSpacing: '-0.02em', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 10 }}>
            <FiVideo size={isMobile ? 20 : 24} style={{ color: T.accent, flexShrink: 0 }} />
            {isMobile ? 'Stress Detection' : 'Cattle Stress Detection'}
          </h1>
          {!isMobile && <p style={{ color: T.subtitle, fontSize: 14 }}>Upload or record a 10-second video to analyze cattle stress levels using AI</p>}
        </div>
      </div>

      {/* ── Mode tabs ── */}
      <div style={{ display: 'flex', gap: 8, marginBottom: isMobile ? 14 : 24 }}>
        {['upload', 'camera'].map(m => (
          <button key={m} onClick={() => { setMode(m); reset(); }} style={{
            ...
            {m === 'upload' ? <><FiUpload size={14} />{isMobile ? 'Upload' : 'Upload Video'}</> : <><FiCamera size={14} />{isMobile ? 'Camera' : 'Record Camera'}</>}
          </button>
        ))}
      </div>

      {/* ── Main Layout ── */}
      <div style={{ display: 'grid', gridTemplateColumns: layoutGrid, gap: isMobile ? 12 : 24, alignItems: 'start' }}>

        {/* ── Left: Input ── */}
        <div>
          {/* Step 1 */}
          <div style={{ ...card, marginBottom: isMobile ? 12 : 16 }}>
            <div style={{ padding: isMobile ? '12px 14px' : '14px 20px', borderBottom: `1px solid ${T.divider}` }}>
              <h3 style={{ fontFamily: F.serif, fontSize: isMobile ? 15 : 17, fontWeight: 700, color: T.title, margin: 0 }}>Step 1: Cattle ID</h3>
            </div>
          ...
          <div style={card}>
            <div style={{ padding: isMobile ? '12px 14px' : '14px 20px', borderBottom: `1px solid ${T.divider}` }}>
              <h3 style={{ fontFamily: F.serif, fontSize: isMobile ? 15 : 17, fontWeight: 700, color: T.title, display: 'flex', alignItems: 'center', gap: 8, margin: 0 }}>
                {mode === 'upload' ? <FiUpload size={16} style={{ color: T.accent }} /> : <FiVideo size={16} style={{ color: T.accent }} />}
            ...
            <div style={{ padding: isMobile ? '14px' : 20 }}>
              {mode === 'upload' ? (
                <>
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: 12, padding: isMobile ? '20px 14px' : '28px 20px',
                      textAlign: 'center',
                  ...
                  >
                    ...
                    <div style={{ marginBottom: 8, display: 'flex', justifyContent: 'center' }}>
                      {step === 'idle' ? <FiUpload size={isMobile ? 28 : 36} style={{ color: T.accent }} /> : <FiCheckCircle size={isMobile ? 28 : 36} style={{ color: '#22c55e' }} />}
                    </div>
                    <div style={{ fontWeight: 700, fontSize: 14, color: T.title, marginBottom: 4 }}>
                      {videoFile ? videoFile.name : 'Drop video here or tap to browse'}
                    </div>
                ...
                <div style={{ textAlign: 'center' }}>
                  <video ref={videoRef} style={{ width: '100%', borderRadius: 10, background: '#000', maxHeight: isMobile ? 160 : 200 }} muted />
                  {!recording ? (
              ...
              {videoURL && (
                <div style={{ marginTop: 16 }}>
                  ...
                  <video src={videoURL} controls style={{ width: '100%', borderRadius: 10, maxHeight: isMobile ? 160 : 180 }} />
                </div>
              )}
            </div>
          </div>
          ...
          {['selected', 'idle'].includes(step) && (
            <button
              ...
              style={{
                ...
                marginTop: isMobile ? 12 : 16, display: 'flex', alignItems: 'center',
        ...
        <div>
          ...
          {processingSteps.includes(step) && (
            <div style={{ ...card, marginBottom: isMobile ? 12 : 16 }}>
              <div style={{ padding: isMobile ? '12px 14px' : '14px 20px', borderBottom: `1px solid ${T.divider}` }}>
                <h3 style={{ fontFamily: F.serif, fontSize: isMobile ? 15 : 17, fontWeight: 700, color: T.title, margin: 0 }}>⚙️ Processing…</h3>
              </div>
              <div style={{ padding: isMobile ? '14px' : 20 }}>
                {[
                  { key: 'uploading', icon: <FiUploadCloud size={16} />, label: 'Uploading video to server', model: '' },
                  { key: 'checking_cattle', icon: <FiCheckSquare size={16} />, label: 'Model 1: Checking cattle presence…', model: 'cow_not_cow.tflite' },
                  ...
                  return (
                    <div key={s.key} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 18 }}>
                      <div style={{
                        width: 32, height: 32, borderRadius: '50%',
                        background: isDone ? T.stepDone : isActive ? T.stepActive : T.stepIdle,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontSize: 14, flexShrink: 0, transition: 'background 0.3s',
                        color: isDone || isActive ? '#fff' : T.muted
                      }}>
                        {isDone ? '✓' : s.icon}
                      </div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 600, fontSize: isMobile ? 12 : 13, color: isActive ? T.accent : isDone ? '#22c55e' : T.muted }}>{s.label}</div>
          ...
          {/* Rejected */}
          {step === 'rejected' && (
            <div style={{ background: isDark ? 'rgba(239,68,68,0.1)' : '#fee2e2', border: '2px solid #fca5a5', borderRadius: 16, padding: isMobile ? 16 : 24 }}>
              <div style={{ textAlign: 'center', marginBottom: 16 }}>
                <div style={{ fontSize: isMobile ? 36 : 48, marginBottom: 8 }}>❌</div>
                <h2 style={{ fontFamily: F.serif, color: '#dc2626', fontSize: isMobile ? 18 : 22, fontWeight: 900 }}>Not a Cattle Video</h2>
              </div>
          ...
          {step === 'done' && result && cfg && (
            <div style={{ background: isDark ? T.cardBg : cfg.bg, border: `2px solid ${cfg.color}`, borderRadius: 16, padding: isMobile ? 16 : 24 }}>
              <div style={{ textAlign: 'center', marginBottom: 18 }}>
                <div style={{ fontSize: isMobile ? 40 : 52, marginBottom: 8 }}>{cfg.icon}</div>
                <div style={{ fontFamily: F.serif, fontSize: isMobile ? 22 : 28, fontWeight: 900, color: cfg.text, letterSpacing: '-0.02em' }}>{result.stress}</div>
                <div style={{ color: cfg.text, opacity: 0.8, fontSize: 13, marginTop: 4 }}>Stress Level {cfg.level} of 5</div>
                <div style={{ display: 'flex', justifyContent: 'center', gap: 6, marginTop: 12 }}>
                  {[1, 2, 3, 4, 5].map(l => (
                    <div key={l} style={{ width: isMobile ? 24 : 32, height: 8, borderRadius: 4, background: l <= cfg.level ? cfg.color : isDark ? '#333' : '#e2e8f0', transition: 'background 0.3s' }} />
                  ))}
                </div>
              </div>
              <div style={{ background: isDark ? '#111' : 'white', borderRadius: 12, padding: isMobile ? 12 : 16, marginBottom: 14 }}>
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '1fr 1fr', gap: 10, marginBottom: 12 }}>
                  {[
                    ...
                    <div key={i} style={{ background: isDark ? '#1a1a1a' : '#f8fafc', borderRadius: 8, padding: '10px 12px' }}>
                      ...
                      <div style={{ fontWeight: 700, fontSize: isMobile ? 13 : 14, color: T.title, marginTop: 2 }}>{item.value}</div>
                    </div>
                  ))}
                </div>
          ...
          {step === 'idle' && (
            <div style={card}>
              <div style={{ padding: isMobile ? '24px 16px' : '36px 24px', textAlign: 'center' }}>
                <div style={{ marginBottom: 14, display: 'flex', justifyContent: 'center' }}>
                  <FiCpu size={isMobile ? 36 : 48} style={{ color: T.accent }} />
                </div>
                <h3 style={{ fontFamily: F.serif, fontSize: isMobile ? 17 : 20, fontWeight: 800, color: T.title, marginBottom: 18 }}>AI Detection Pipeline</h3>
                <div style={{ textAlign: 'left' }}>
                  {[
                    ...
                    { icon: <FiBarChart2 size={15} />, step: '4', label: 'Results saved & recommendations', sub: '5-level classification' },
                  ].map((item, i) => (
                    <div key={i} style={{ display: 'flex', gap: 12, marginBottom: 16, alignItems: 'flex-start' }}>
                      <div style={{ width: 28, height: 28, borderRadius: '50%', background: T.primaryBg, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
                        {item.step}
                      </div>
                      <div>
                        <div style={{ fontWeight: 600, fontSize: isMobile ? 12 : 13, color: T.title, display: 'flex', alignItems: 'center', gap: 7 }}>
